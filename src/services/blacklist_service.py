...
class BlacklistService:
    ...
    async def add_to_blacklist(self, blacklist_data: BlacklistCreate) -> Blacklist:
        ...
        expires_at = None
        if (blacklist_data.blacklist_type == BlacklistType.TEMPORARY and 
            blacklist_data.duration_seconds):
        ...
        logger.info(f"Added to {blacklist_data.blacklist_type} blacklist: {blacklist_data.group_id} - {blacklist_data.reason}")
        
        return blacklist
    
    async def add_from_error(self, group_id: str, error_msg: str, group_identifier: str = None) -> Blacklist:
    ...
    async def get_blacklist_stats(self) -> dict:
        ...
        permanent = await self.collection.count_documents({"blacklist_type": BlacklistType.PERMANENT})
        temporary = await self.collection.count_documents({"blacklist_type": BlacklistType.TEMPORARY})
