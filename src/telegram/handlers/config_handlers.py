...
class ConfigHandlers:
                        ...
                        if config.is_editable:
                            keyboard.append([
                                InlineKeyboardButton(
                                    f"✏️ {config.key}",
                                    callback_data=f"config_edit_{config.id}"
                                )
                            ])
                    text += "\n"
        ...
        try:
            ...
            if waiting_for.startswith('config_'):
                ...
                try:
                    ...
                    updated_config = await self.config_service.set_config(config.key, converted_value)
    ...
    async def _show_edit_config(self, update: Update, context: ContextTypes.DEFAULT_TYPE, config_id: str):
        ...
        try:
            ...
            if not config.is_editable:
                await update.callback_query.edit_message_text("❌ Konfigurasi ini tidak dapat diedit.")
                return
