...
class MessageHandlers:
            ...
            if not messages:
                ...
                for i, msg in enumerate(messages[:10], 1):  # Show first 10
                    ...
                    keyboard.append([
                        ...
                        InlineKeyboardButton(f"🗑️ Hapus #{i}", callback_data=f"messages_delete_{msg.id}")
                    ])
                
                if len(messages) > 10:
                    text += f"... dan {len(messages) - 10} pesan lainnya\n\n"
                
                keyboard.append([InlineKeyboardButton("➕ Tambah Pesan", callback_data="messages_add")])
    ...
    async def handle_message_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if len(content) > 4096:
                await update.message.reply_text(f"❌ Pesan terlalu panjang ({len(content)} karakter). Maksimal 4096 karakter. Coba lagi:")
                return
    ...
    async def _confirm_delete_message(self, update: Update, message_id: str):
        ...
        keyboard = [
            [
                InlineKeyboardButton("✅ Ya, Hapus", callback_data=f"messages_delete_confirm_{message_id}"),
