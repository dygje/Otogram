...
class UserBot:
    ...
    async def _broadcast_cycle(self, messages: list, groups: list):
        ...
        for group in groups:
            ...
            if await self.blacklist_service.is_blacklisted(group.group_id or str(group.id)):
                skipped_count += 1
                logger.debug(
                    f"⏭️ Skipping blacklisted group: {group.group_id or group.group_username}"
                )
                continue
    ...
    async def _send_message_to_group(self, group, message_content: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"⏰ {error_type} detected for {chat_id}: {duration}s ({time_str}) - SKIPPING and adding to temporary blacklist"
            )
            return False
